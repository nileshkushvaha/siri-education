<div class="space-y-6">
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        @if($dashboard->wallet)<a href="{{ route('dashboard.wallet') }}" class="rounded-2xl border border-edge bg-surface-raised p-5 hover:bg-surface-hover"><p class="text-sm text-fg-muted">Wallet</p><p class="mt-2 text-xl font-bold text-emerald-600 dark:text-emerald-300">{{ $dashboard->wallet['available'] }}</p>@if($dashboard->wallet['low_balance'] ?? false)<p class="mt-2 text-xs font-semibold text-amber-600 dark:text-amber-300">Low balance — recharge soon</p>@else<p class="mt-2 truncate text-xs text-fg-faint">{{ $dashboard->wallet['latest'] ?: 'No recent wallet activity' }}</p>@endif</a>@endif

...
**Template-rendered routes** (home template, `/blog`, `/instructors`, `/faqs`, `/login`, `/register`, `/become-instructor`, `/forgot-password`) do not go through a CMS Page. Their `<title>`, `meta description`, `og:title` and `og:description` are resolved once in `layouts/frontend.blade.php` by `SeoManager::getRouteMetadata()`: the matching entry in the **Page-specific SEO** section of Admin → SEO Settings (`SeoSettings::$pages`, keyed by `SeoRoute`; one page is edited at a time via a selector, with meta title, description, keywords, canonical URL and Open Graph image) wins; the home page then falls back to the global SEO Settings meta title/description; finally the template's own `@section('title')` / `@section('meta_description')` apply. Keywords and the OG image fall back to SEO Settings, and the canonical link defaults to the page's own URL. Templates must set those sections rather than pushing their own `<meta name="description">` or canonical link, so each page carries exactly one of each. A CMS page marked as homepage is rendered by `ContentRenderer` and keeps using its own SEO fields.

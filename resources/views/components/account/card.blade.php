...
<div {{ $attributes->class(['relative overflow-hidden rounded-2xl border border-white/[0.075] p-5 sm:p-6 group transition-all duration-300'])->merge(['data-account-card' => '']) }}>
    ...
    @if($title)
        <div class="relative flex items-center justify-between gap-4 mb-6">
            ...
            @if($linkText && $linkHref)
                <a href="{{ $linkHref }}" class="text-xs font-semibold text-[rgb(var(--portal-a))] transition hover:text-white">{{ $linkText }}</a>
            @endif
        </div>
    @endif

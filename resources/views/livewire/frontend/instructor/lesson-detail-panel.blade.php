...
<div class="mt-4 rounded-xl border border-edge bg-surface-raised p-4">
    <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-fg-faint">Lesson details</p>
    <dl class="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Student</dt>
            <dd class="text-fg-muted">{{ $lesson->student?->name ?? 'Student' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Subject</dt>
            <dd class="text-fg-muted">{{ $lesson->subject?->name ?? 'General' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Date &amp; time</dt>
            <dd class="text-fg-muted">{{ $lesson->starts_at->copy()->timezone($timezone)->format('M j, Y g:i A') }}</dd>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Duration</dt>
            <dd class="text-fg-muted">{{ $lesson->starts_at->diffInMinutes($lesson->ends_at) }} minutes</dd>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Meeting status</dt>
            <dd class="text-fg-muted">{{ $lesson->booking?->meeting?->status?->label() ?? 'Not scheduled' }}</dd>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Lesson status</dt>
            <dd><x-ui.badge :color="$lesson->status->color()">{{ $lesson->status->label() }}</x-ui.badge></dd>
        </div>
        @if($homeworkStatus)
            <div>
                <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Homework</dt>
                <dd class="text-fg-muted">{{ $homeworkStatus }}</dd>
            </div>
        @endif
    </dl>

    @if($acceptsSubmissions || $canConfirmOutcome)
        <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-edge pt-4">
            @if($acceptsSubmissions)
                ...
                @if($reportingIssueId !== $lesson->id)
                    <button type="button" wire:click="startReportIssue('{{ $lesson->id }}')"
                            class="min-h-11 px-4 py-2 rounded-xl text-xs font-semibold text-fg-muted border border-edge hover:bg-surface-hover transition"
    ...
    @if($reportingIssueId === $lesson->id)
        <form wire:submit="submitIssueReport('{{ $lesson->id }}')" class="mt-4 space-y-3 border-t border-edge pt-4">
            <div>
                <label for="issue-category-{{ $lesson->id }}" class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Issue type</label>
                ...
                @error('issue_category') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
            </div>
            <div>
                <label for="issue-description-{{ $lesson->id }}" class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Description (optional)</label>
                ...
                @error('issue_description') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
            </div>
            <div class="flex items-center gap-3">
                ...
                <button type="button" wire:click="cancelReportIssue"
                        class="min-h-11 px-4 py-2 rounded-xl text-sm font-semibold text-fg-muted border border-edge hover:bg-surface-hover transition">
                    Cancel
                </button>
            </div>
        </form>
    ...
    @if($lesson->outcome === $completedOutcome && $lesson->hasFinalizedOutcome())
        ...
        <div class="mt-4 border-t border-edge pt-4">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-fg-faint">Lesson summary</p>

            @if($aiSummary?->status === \App\Lessons\Summaries\Enums\LessonSummaryStatus::Approved)
                <p class="text-sm text-fg whitespace-pre-line">{{ $aiSummary->approved_summary }}</p>
            ...
            @elseif($aiSummary?->status === \App\Lessons\Summaries\Enums\LessonSummaryStatus::Ready)
                @if($summaryEditingId === $aiSummary->id)
                    ...
                    <form wire:submit="approveSummary" class="space-y-3">
                        <textarea wire:model="summaryText" rows="5"
                                  class="w-full rounded-xl bg-surface-raised border border-edge text-sm text-fg px-3 py-2 focus:outline-none focus:border-emerald-500/40"></textarea>
                        @error('summaryText')
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button type="button" wire:click="cancelSummaryReview"
                                    class="min-h-11 px-4 py-2 rounded-xl text-xs font-medium text-fg-muted hover:text-fg-strong transition">
                                Cancel
                            </button>
                        </div>
                    </form>
                @else
                    ...
                    <p class="text-sm text-fg-muted whitespace-pre-line">{{ $aiSummary->lesson_summary }}</p>
                    ...
                    @foreach ([
                        ...
                        @if(filled($items))
                            <p class="mt-3 text-[11px] font-semibold uppercase tracking-wide text-fg-faint">{{ $heading }}</p>
                            <ul class="mt-1 space-y-0.5">
                                @foreach($items as $item)
                                    <li class="text-xs text-fg-muted">&bull; {{ $item }}</li>
                                @endforeach
                            </ul>
                        @endif
                    @endforeach
                    ...
                    <div class="flex items-center gap-2 mt-3">
                        <button type="button" wire:click="startSummaryReview('{{ $aiSummary->id }}')"
                                class="min-h-11 px-4 py-2 rounded-xl text-xs font-semibold text-emerald-700 dark:text-emerald-200 bg-emerald-500/10 border border-emerald-400/20 hover:bg-emerald-500/20 transition">
                            Review &amp; approve
                        </button>
            ...
            @else
                ...
                <button type="button" wire:click="generateSummary('{{ $lesson->id }}')"
                        ...
                        class="mt-2 min-h-11 px-4 py-2 rounded-xl text-xs font-semibold text-emerald-700 dark:text-emerald-200 bg-emerald-500/10 border border-emerald-400/20 hover:bg-emerald-500/20 transition">
                    Generate AI summary
                </button>
                @error('aiSummary')
    ...
    @if($lesson->outcome === $completedOutcome && $lesson->hasFinalizedOutcome())
        ...
        <div class="mt-4 border-t border-edge pt-4">
            @if($feedback)
                <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-fg-faint">Private feedback &middot; visible only to you</p>
                <dl class="space-y-3 text-sm">
                    @if($feedback->attendanceStatusSnapshot)
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Finalized attendance</dt>
                            <dd class="text-fg-muted">{{ $feedback->attendanceStatusSnapshot->label() }}</dd>
                        </div>
                    @endif
                    @foreach ([
                        ...
                        @if($feedback->{$property})
                            <div>
                                ...
                                <dd class="text-fg-muted">{{ $feedback->{$property} }}</dd>
                            </div>
                        @endif
                    @endforeach
                </dl>
                <p class="mt-4 text-xs text-fg-faint">Submitted {{ $feedback->submittedAt->copy()->timezone($timezone)->format('M j, Y g:i A') }}. Feedback cannot be edited or deleted.</p>
            @else
                <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-fg-faint">Private feedback &middot; visible only to you</p>
                <form wire:submit="submitFeedback('{{ $lesson->id }}')" class="space-y-4">
                    @if($lesson->student_attendance_status)
                        <p class="text-xs text-fg-faint">Finalized attendance: <span class="text-fg-muted">{{ $lesson->student_attendance_status->label() }}</span> (not editable here)</p>
                    @endif
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Attendance observation</label>
                        <textarea wire:model="attendance_observation" rows="2" maxlength="1000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('attendance_observation') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Preparedness</label>
                        <textarea wire:model="preparedness_observation" rows="2" maxlength="1000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('preparedness_observation') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Homework completion</label>
                        <textarea wire:model="homework_completion_observation" rows="2" maxlength="1000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('homework_completion_observation') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Engagement</label>
                        <textarea wire:model="engagement_observation" rows="2" maxlength="1000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('engagement_observation') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Learning attitude</label>
                        <textarea wire:model="learning_attitude_observation" rows="2" maxlength="1000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('learning_attitude_observation') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Areas needing support</label>
                        <textarea wire:model="areas_needing_support" rows="2" maxlength="1000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('areas_needing_support') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Private notes for learning continuity</label>
                        <textarea wire:model="private_notes" rows="3" maxlength="2000"
                            class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none"></textarea>
                        @error('private_notes') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                    </div>

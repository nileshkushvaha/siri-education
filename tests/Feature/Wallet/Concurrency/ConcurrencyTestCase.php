...
/**
 * Base for the Wallet domain's real-MySQL concurrency tests — mirrors
 * Tests\Feature\Booking\Concurrency\ConcurrencyTestCase's shape
 * (itself mirroring Tests\Feature\Earnings\Concurrency\ConcurrencyTestCase),
 * including its migrate:fresh-with-bounded-retry teardown fix: a
 * transient migration failure right after a race's child processes
 * exit (their MySQL locks are not always guaranteed released the
 * instant the client process exits) must fail loudly, not silently
 * leave the next test class inheriting a partially-rebuilt database.
 */

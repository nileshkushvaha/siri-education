<div class="space-y-6">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <x-account.stat-card label="Upcoming lesson" :value="$summary->nextLessonAt?->format('M j, Y') ?? 'None'" gradient="from-amber-500 to-orange-500"
                ...
                <div wire:key="recent-lesson-{{ $lesson['id'] }}" class="py-4 {{ ! $loop->last ? 'border-b border-white/[0.05]' : '' }}">
                    <div class="flex items-center justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-400">{{ $lesson['starts_at']->format('M j, Y g:i A') }}</p>
                        </div>

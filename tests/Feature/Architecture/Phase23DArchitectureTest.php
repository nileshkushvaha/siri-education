...
final class Phase23DArchitectureTest extends TestCase
{
    public function test_only_the_established_authorities_assign_instructor_status_directly(): void
    {
        $allowed = [
            ...
            // and is unrelated to the six lifecycle transitions. It moved
            // out of EditUser.php with the rest of the lifecycle actions
            // when instructor review split off into
            // InstructorOnboardingResource (see docs/users.md); the
            // override is the same single exception, in its new home.
    ...
    {
        // The six actions live in the trait now, not on EditUser — the
        // rule they encode ("call the service, never $model->update()")
        // is unchanged, so this follows them rather than being relaxed.
        $lifecycleActions = file_get_contents(app_path('Filament/Concerns/HasInstructorLifecycleActions.php'));
        $this->assertIsString($lifecycleActions);
        ...
        foreach ([
            ...
            $body = $this->methodBody($lifecycleActions, $method);

            $this->assertNotNull($body, "Could not locate method {$method}() in HasInstructorLifecycleActions.php.");

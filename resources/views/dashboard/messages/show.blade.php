...
@section('account-content')
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-fg-strong">{{ $other?->name }}</h1>
            <p class="text-fg-muted text-sm mt-1">
                Context:
                @if($conversation->context_type === \App\Models\Booking::class)
                    <a href="{{ ($accountAudience ?? null) === \App\Enums\PortalAudience::Instructor ? route('dashboard.instructor.lessons') : route('dashboard.my-bookings') }}" class="underline hover:text-fg-strong">Related booking</a>
                @else
                    <a href="{{ ($accountAudience ?? null) === \App\Enums\PortalAudience::Instructor ? route('dashboard.instructor.learning-plans') : route('dashboard.learning-plans') }}" class="underline hover:text-fg-strong">Related learning plan</a>
                @endif
            </p>
        </div>
    ...
    <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-5 sm:p-7 mb-5">
        <div class="space-y-4">
            @forelse($conversation->messages as $msg)
                ...
                <div wire:key="message-{{ $msg->id }}" class="{{ !$loop->last ? 'pb-4 border-b border-edge' : '' }}">
                    <div class="flex items-center gap-2 mb-1">
                        <p class="text-xs font-semibold {{ $isMine ? 'text-indigo-600 dark:text-indigo-300' : 'text-fg-strong' }}">{{ $isMine ? 'You' : $msg->sender->name }}</p>
                        <p class="text-xs text-fg-faint">{{ viewer_datetime($msg->sent_at) }}</p>
                    </div>
                    <p class="text-sm text-fg-muted whitespace-pre-line">{{ $msg->body }}</p>

                    @if($msg->getFirstMedia('attachment'))
                        <a href="{{ route('dashboard.media.download', $msg->getFirstMedia('attachment')) }}" class="mt-1 inline-flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-300 underline" target="_blank" rel="noopener">
                    ...
                    @if(!$isMine)
                        <details class="mt-2">
                            <summary class="text-xs text-fg-faint cursor-pointer hover:text-fg-muted">Report this message</summary>
                            <form method="POST" action="{{ route('dashboard.messages.report', [$conversation, $msg]) }}" class="mt-2 flex flex-col gap-2 max-w-sm">
                                @csrf
                                <select name="reason" required class="min-h-11 px-3 py-2 rounded-lg bg-surface-raised border border-edge text-fg text-xs">
                                    @foreach(\App\Messaging\Enums\MessageReportReason::cases() as $reason)
                                        <option value="{{ $reason->value }}">{{ $reason->label() }}</option>
                                    @endforeach
                                </select>
                                <textarea name="details" rows="2" maxlength="1000" placeholder="Optional details" class="px-3 py-2 rounded-lg bg-surface-raised border border-edge text-fg text-xs"></textarea>
                                <button type="submit" class="self-start inline-flex min-h-9 items-center px-3 py-1.5 rounded-lg bg-surface-raised text-xs font-semibold text-fg-strong hover:bg-surface-hover">
                                    Submit report
                                </button>
                            </form>
                        </details>
            ...
            @empty
                <p class="text-sm text-fg-muted">No messages yet.</p>
            @endforelse
        </div>
        ...
        @can('reply', $conversation)
            ...
            @if (session('safety_warning'))
                <div class="mt-6 rounded-xl border border-amber-400/25 bg-amber-500/[0.07] px-4 py-3">
                    <p class="text-sm font-semibold text-amber-700 dark:text-amber-200">Your message looks like it contains {{ session('safety_warning') }}.</p>
            ...
            <form method="POST" action="{{ route('dashboard.messages.reply', $conversation) }}" enctype="multipart/form-data" class="mt-6 pt-5 border-t border-edge">
                @csrf
                <label for="body" class="block text-xs font-semibold text-fg-muted mb-2">Reply</label>
                ...
                <div class="mt-3">
                    <label for="attachment" class="block text-xs font-semibold text-fg-muted mb-2">Attachment (PDF or image, optional)</label>
                    <input type="file" id="attachment" name="attachment" accept=".pdf,.jpg,.jpeg,.png,.webp" class="text-xs text-fg-muted">
                ...
                @if (session('safety_warning'))
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        <span class="text-xs text-fg-muted">or edit your message above</span>
                    </div>
                @else
                    <button type="submit"
        ...
        @else
            <p class="mt-6 pt-5 border-t border-edge text-sm text-fg-muted">This conversation is {{ strtolower($conversation->status->label()) }} and cannot accept new messages.</p>
        @endcan
    </div>

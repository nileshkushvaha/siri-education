    ...
    <div class="flex items-center gap-1" style="padding-left: {{ $depth * 0.75 }}rem">
        @if(! $isEmpty)
            <a
                ...
                class="flex min-h-11 flex-1 items-center justify-between rounded-xl px-3 text-sm font-bold transition {{ $node->isActive ? 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-100' : 'text-slate-700 hover:bg-slate-100' }}"
        ...
        @else
            <span class="flex min-h-11 flex-1 items-center rounded-xl px-3 text-sm font-semibold text-slate-500">{{ $node->label }}</span>
        @endif

        @if($hasChildren)
            <button type="button" x-on:click="open = ! open" x-bind:aria-expanded="open" aria-controls="{{ $panelId }}" class="flex h-11 w-11 items-center justify-center rounded-xl text-slate-500 transition hover:bg-indigo-50 hover:text-indigo-600 focus:outline-none focus-visible:ring-4 focus-visible:ring-indigo-100">

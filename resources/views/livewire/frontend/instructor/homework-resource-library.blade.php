<div>
    ...
    <div class="mb-6 flex flex-wrap items-center gap-3">
        <input type="text" wire:model.live.debounce.400ms="search" placeholder="Search resources…"
               class="rounded-xl bg-surface-raised border border-edge text-sm text-fg px-3 py-2 focus:outline-none focus:border-indigo-500/40" />

        <select wire:model.live="statusFilter" class="rounded-xl bg-surface-raised border border-edge text-sm text-fg px-3 py-2 focus:outline-none focus:border-indigo-500/40">
        ...
        <x-account.card :title="'New Resource'">
            <div class="grid gap-3">
                <div>
                    ...
                    @error('createTitle')<p class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p>@enderror
                </div>
                ...
                <div>
                    ...
                    @error('createDescription')<p class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p>@enderror
                </div>

                <div class="grid gap-3 sm:grid-cols-2">
                    <div>
                        <label class="block text-xs text-fg-muted mb-1">Subject (optional)</label>
                        <select wire:model="createSubjectId" class="w-full rounded-xl bg-surface-raised border border-edge text-sm text-fg px-3 py-2 focus:outline-none focus:border-indigo-500/40">
                    ...
                    <div>
                        <label class="block text-xs text-fg-muted mb-1">Level (optional)</label>
                        <select wire:model="createAcademicLevelId" class="w-full rounded-xl bg-surface-raised border border-edge text-sm text-fg px-3 py-2 focus:outline-none focus:border-indigo-500/40">
                ...
                @error('createSubjectId')<p class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p>@enderror
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button wire:click="cancelCreate" class="px-4 py-2 rounded-xl text-xs font-medium text-fg-muted hover:text-fg-strong transition-all">
                        Cancel
                    </button>
                </div>
            </div>
    ...
    <div class="mt-6">
        <x-account.card>
            @forelse($resources as $resource)
                <div wire:key="resource-{{ $resource->id }}" class="py-4 {{ !$loop->last ? 'border-b border-edge' : '' }}">
                    <div class="flex items-center justify-between gap-4">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <p class="text-sm font-medium text-fg-strong truncate">{{ $resource->title }}</p>
                        ...
                        <div class="flex-shrink-0 flex items-center gap-2">
                            <button wire:click="toggleHistory('{{ $resource->id }}')" class="text-xs text-indigo-600 dark:text-indigo-300 underline">
                            ...
                            @if($resource->status->value === 'active')
                                <button wire:click="archiveResource('{{ $resource->id }}')" wire:confirm="Archive this resource? It can no longer be attached to new homework."
                                        class="text-xs text-rose-600 dark:text-rose-400 hover:text-rose-600 hover:dark:text-rose-300">
                                    Archive
                                </button>
                            @endif
                        </div>
                    ...
                    @if($expandedResourceId === $resource->id)
                        <div class="mt-3 pl-0">
                            @foreach($resource->versions as $version)
                                <div class="flex items-center gap-2 text-xs text-fg-muted mb-1.5" wire:key="version-{{ $version->id }}">
                                    ...
                                    @endif
                                    <span class="text-fg-faint">{{ viewer_datetime($version->published_at) }}</span>

                                    @if($resource->status->value === 'active')
                                        <button wire:click="startAttach('{{ $version->id }}')" class="text-indigo-600 dark:text-indigo-300 underline">Attach to homework</button>
                                    @endif
                                </div>

                                @if($attachingVersionId === $version->id)
                                    <div class="mb-3 rounded-xl bg-surface-raised p-3">
                                        <select wire:model="attachAssignmentId" class="w-full rounded-xl bg-surface-raised border border-edge text-sm text-fg px-3 py-2 focus:outline-none focus:border-indigo-500/40">
                                            <option value="">Choose an assignment…</option>
                                            @foreach($assignableAssignments as $assignmentId => $label)
                                                <option value="{{ $assignmentId }}">{{ $label }}</option>
                                            @endforeach
                                        </select>
                                        @error('attachAssignmentId')<p class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p>@enderror
                                        ...
                                        <div class="flex items-center gap-2 mt-2">
                                            ...
                                            <button wire:click="cancelAttach" class="px-3 py-1.5 rounded-lg text-xs font-medium text-fg-muted hover:text-fg-strong transition-all">
                                                Cancel
                                            </button>
                                        </div>
                                    </div>
                            ...
                            @if($resource->status->value === 'active')
                                @if($publishingResourceId === $resource->id)
                                    <div class="mt-2">
                                        <input type="file" wire:model="newVersionFile" accept=".pdf,.jpg,.jpeg,.png,.webp" class="text-xs text-fg-muted">
                                        @error('newVersionFile')<p class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p>@enderror
                                        <div class="flex items-center gap-2 mt-2">
                                            ...
                                            <button wire:click="cancelPublish" class="px-3 py-1.5 rounded-lg text-xs font-medium text-fg-muted hover:text-fg-strong transition-all">
                                                Cancel
                                            </button>
                                        </div>
                                    </div>
                                @else
                                    <button wire:click="startPublish('{{ $resource->id }}')" class="mt-1 text-xs text-indigo-600 dark:text-indigo-300 underline">
                                        + Publish new version
                                    </button>
                                @endif
                            @endif

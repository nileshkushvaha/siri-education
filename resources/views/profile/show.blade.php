...
<div x-data="{
    ...
    <x-account.profile-header :summary="$accountProfileSummary" variant="full">
        <x-slot:avatar>
            ...
            <template x-if="!avatarPreview">
                <span class="text-3xl font-bold text-fg-strong">{{ $accountProfileSummary->initial }}</span>
            </template>
        </x-slot:avatar>
        <x-slot:avatarActions>
            ...
            <label class="absolute inset-0 rounded-2xl flex items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"
                ...
                <svg x-show="!uploading" class="w-6 h-6 text-fg-strong" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <svg x-show="uploading" class="w-6 h-6 text-fg-strong animate-spin" fill="none" viewBox="0 0 24 24" style="display:none">
        ...
        <x-slot:coverActions>
            <label class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover/cover:opacity-100 cursor-pointer transition-opacity"
                ...
                <span class="flex items-center gap-2 px-4 py-2 rounded-xl bg-black/50 text-fg-strong text-xs font-semibold">
            ...
            <button x-show="coverPreview" @click="deleteCover()" type="button"
                class="absolute top-3 right-3 px-3 py-1.5 rounded-lg bg-black/50 text-fg-strong text-xs font-medium hover:bg-red-500/60 transition-colors"
                style="display:none">
                Remove
            </button>
        </x-slot:coverActions>
        ...
        <p class="text-fg-muted text-xs mt-1">{{ $user->profile->country->flag }} {{ $user->profile->country->name }}</p>
        @endif
        <p x-show="uploadError" x-text="uploadError" class="text-xs text-red-600 dark:text-red-400 mt-2"></p>
        <p x-show="coverUploadError" x-text="coverUploadError" class="text-xs text-red-600 dark:text-red-400 mt-2"></p>
        <button x-show="avatarPreview" @click="deleteAvatar()" type="button"
            class="mt-2 flex items-center gap-1 text-xs text-fg-muted hover:text-red-600 hover:dark:text-red-400 transition-colors"
            style="display:none">
        ...
        <x-slot:actions>
            <div class="text-xs text-fg-muted text-right">Hover photos to change</div>
        </x-slot:actions>
    ...
    <section class="mb-6 overflow-hidden rounded-2xl border border-indigo-400/15 bg-gradient-to-r from-indigo-500/[0.10] via-violet-500/[0.05] to-transparent p-5 sm:p-6" aria-labelledby="profile-workspace-title">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-600 dark:text-indigo-300">{{ $portalAudience === \App\Enums\PortalAudience::Instructor ? 'Instructor profile' : 'Student profile' }}</p>
                <h1 id="profile-workspace-title" class="mt-1 text-xl font-bold text-fg-strong">{{ $portalAudience === \App\Enums\PortalAudience::Instructor ? 'Build trust with a complete teaching profile' : 'Personalize your learning experience' }}</h1>
                <p class="mt-2 max-w-2xl text-sm leading-6 text-fg-muted">{{ $portalAudience === \App\Enums\PortalAudience::Instructor ? 'Keep your professional story accurate here, then manage teaching credentials and preferences in instructor setup.' : 'Your academic level, preferred subjects, language, and timezone help us tailor lessons and recommendations.' }}</p>
            </div>
            ...
            @else
            <a href="{{ route('dashboard.learning-goals') }}" class="inline-flex min-h-11 flex-shrink-0 items-center justify-center rounded-xl border border-edge bg-surface-raised px-4 text-sm font-semibold text-fg-strong hover:bg-surface-hover focus:outline-none focus:ring-2 focus:ring-indigo-300">Manage learning goals</a>
            @endif
        </div>
    </section>
    ...
    <div>
        ...
        <div class="flex items-center gap-1 overflow-x-auto rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-1.5 mb-6">
            @foreach([
            ...
            <button type="button" @click="activeTab = '{{ $tab }}'"
                :class="activeTab === '{{ $tab }}'
                    ...
                    : 'text-fg-muted hover:text-fg-strong hover:bg-surface-hover'"
        ...
        <div class="space-y-5">
            ...
            <div x-show="activeTab === 'general'"
                ...
                {{-- ── Appearance (portal theme) ─────────────────────── --}}
                <form method="POST" action="{{ route('profile.theme.update') }}" class="mb-5 rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-5 sm:p-7">
                    @csrf
                    @php $currentTheme = app(\App\Services\ThemeResolver::class)->resolve($user); @endphp
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            <h2 class="text-base font-semibold text-fg-strong">Appearance</h2>
                            <p class="mt-1 text-sm text-fg-muted">Choose how your portal looks. Light is the default; "Match my device" follows your system setting.</p>
                        </div>
                        <div class="flex items-center gap-3">
                            <label for="theme_preference" class="sr-only">Theme</label>
                            <select id="theme_preference" name="theme" class="min-h-11 rounded-xl border border-edge bg-surface-raised px-3 text-sm text-fg-strong focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
                                    onchange="window.portalTheme?.set(this.value, { persist: false })">
                                @foreach(\App\Enums\ThemePreference::options() as $value => $label)
                                    <option value="{{ $value }}" @selected($currentTheme->value === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                            <button type="submit" class="inline-flex min-h-11 items-center rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white hover:bg-indigo-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400">Save</button>
                        </div>
                    </div>
                    @error('theme')<p class="mt-2 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p>@enderror
                </form>
                ...
                <form method="POST" action="{{ route('profile.update') }}">
                    ...
                    <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-5 sm:p-7 mb-5">
                        <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                            ...
                            <div>
                                <h2 class="text-base font-semibold text-fg-strong">Personal Information</h2>
                                <p class="text-xs text-fg-muted">Your name, contact, and identity details</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                            {{-- First Name --}}
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">First Name <span class="text-red-600 dark:text-red-400">*</span></label>
                                <input type="text" name="first_name" value="{{ old('first_name', $user->first_name) }}" autocomplete="given-name"
                                    class="w-full px-4 py-3 rounded-xl bg-surface-raised border @error('first_name') border-red-500/50 @else border-edge @enderror text-fg placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/30 transition-all"
                            ...
                            {{-- Last Name --}}
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Last Name</label>
                            ...
                            <div class="sm:col-span-2 lg:col-span-3">
                                <h3 class="text-sm font-semibold text-fg-strong mb-3">Contact and regional settings</h3>
                                <div class="grid gap-3 sm:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                                    <div>
                                        <label for="phone_country_iso2" class="block text-xs font-semibold text-fg-muted mb-2">Phone country</label>
                                        <select id="phone_country_iso2" name="phone_country_iso2" x-model="selectedPhoneCountry" autocomplete="tel-country-code" class="w-full min-h-11 px-4 py-3 rounded-xl bg-surface-raised border border-edge text-fg text-sm focus:ring-2 focus:ring-indigo-500/30">
                                    ...
                                    <div>
                                        <label for="phone" class="block text-xs font-semibold text-fg-muted mb-2">Mobile number</label>
                                        <input id="phone" type="tel" name="phone" value="{{ old('phone', $user->profile->phone_national_number) }}" autocomplete="tel-national" maxlength="20"
                                            class="w-full min-h-11 px-4 py-3 rounded-xl bg-surface-raised border @error('phone') border-red-500/50 @else border-edge @enderror text-fg text-sm focus:ring-2 focus:ring-indigo-500/30" :placeholder="phonePlaceholder">
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Email Address</label>
                                <div class="relative">
                                    <input type="email" value="{{ $user->email }}" readonly disabled
                                        class="w-full px-4 py-3 rounded-xl bg-surface-raised border border-edge text-fg-muted text-sm cursor-not-allowed pr-10">
                                ...
                                <p class="mt-1.5 text-xs text-fg-muted">Your email address cannot be changed. Contact support if you need to update it.</p>
                            </div>
                            ...
                            {{-- Date of Birth --}}
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Date of Birth</label>
                                <input type="date" name="date_of_birth"
                                    value="{{ old('date_of_birth', $user->profile->date_of_birth?->format('Y-m-d')) }}"
                                    max="{{ now()->subDay()->format('Y-m-d') }}" autocomplete="bday"
                                    class="w-full px-4 py-3 rounded-xl bg-surface-raised border border-edge text-fg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/30 transition-all [color-scheme:dark]">
                            </div>
                        </div>

                        @if($portalAudience === \App\Enums\PortalAudience::Instructor)
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-5 border-t border-edge pt-5">
                            {{-- Headline --}}
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Headline</label>
                            ...
                            {{-- Bio --}}
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Bio</label>
                            ...
                            <div class="sm:col-span-2 rounded-xl border border-indigo-400/15 bg-indigo-500/[0.05] p-4 text-sm leading-6 text-fg-muted">Teaching philosophy, subjects, education levels, languages, experience, education, certifications, introduction video, and verification documents are managed in <a href="{{ route('dashboard.instructor.onboarding') }}" class="font-semibold text-indigo-600 dark:text-indigo-300 hover:text-indigo-700 hover:dark:text-indigo-200">Teaching profile setup</a>.</div>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-5 sm:p-7 mb-5">
                        <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                            ...
                            <div>
                                <h2 class="text-base font-semibold text-fg-strong">Learning Profile</h2>
                                <p class="text-xs text-fg-muted">Used for learning goals, dashboard guidance, and future instructor recommendations</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                ...
                                @error('student_academic_level_id')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('student_preferred_language_id')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                            </div>
                        </div>

                        <div class="mt-5">
                            <label class="block text-xs font-semibold text-fg-muted mb-2">Preferred Subjects</label>
                            @php
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                <label class="flex items-center gap-3 rounded-xl border border-edge bg-surface-raised px-4 py-3 text-sm text-fg">
                                    ...
                                    class="rounded border-edge-strong bg-surface-solid text-indigo-600 focus:ring-indigo-500/40">
                                    <span>{{ $subject->name }}</span>
                                </label>
                                @endforeach
                            </div>
                            <p class="mt-2 text-xs text-fg-faint">Subjects come from the academic master catalog, not free text.</p>
                            @error('preferred_subject_ids')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                            @error('preferred_subject_ids.*')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-5 sm:p-7 mb-5">
                        <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                            ...
                            <div>
                                <h2 class="text-base font-semibold text-fg-strong">Region &amp; Timezone</h2>
                                <p class="text-xs text-fg-muted">Controls lesson times{{ $portalAudience === \App\Enums\PortalAudience::Student ? ', pricing, and billing currency' : ' and regional availability' }}</p>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                            <div class="sm:col-span-2 lg:col-span-3">
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Street Address</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Country of residence</label>
                                ...
                                @if($portalAudience === \App\Enums\PortalAudience::Student)
                                <p class="mt-2 text-xs {{ $billingCountryChangeBlocked ? 'text-amber-600 dark:text-amber-300' : 'text-fg-muted' }}">{{ $billingCountryChangeBlocked ? 'Locked because active classes or wallet history exist. Contact support to request a change.' : 'Your billing currency is derived from this country.' }}</p>
                                @endif
                                @error('country_id')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">City</label>
                                <input type="text" name="city" value="{{ old('city', $user->profile->city) }}" autocomplete="address-level2"
                                    class="w-full px-4 py-3 rounded-xl bg-surface-raised border border-edge text-fg placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/30 transition-all" placeholder="Enter your city">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Postal Code</label>
                                <input type="text" name="postal_code" value="{{ old('postal_code', $user->profile->postal_code) }}" autocomplete="postal-code"
                                    class="w-full px-4 py-3 rounded-xl bg-surface-raised border border-edge text-fg placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/30 transition-all" placeholder="Enter postal or ZIP code">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Timezone <span class="text-red-600 dark:text-red-400">*</span></label>
                                <select name="timezone" required class="w-full px-4 py-3 rounded-xl bg-surface-raised border @error('timezone') border-red-500/50 @else border-edge @enderror text-fg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 appearance-none">
                                ...
                                <p class="mt-2 text-xs text-fg-muted">All lesson schedules are displayed in this timezone.</p>
                                @error('timezone')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-fg-muted mb-2">Interface Language</label>
                                <select name="language" class="w-full px-4 py-3 rounded-xl bg-surface-raised border border-edge text-fg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 appearance-none">
                ...
                <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-7 mt-5">
                    <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                        <div class="w-9 h-9 rounded-xl bg-emerald-500/15 border border-emerald-500/25 flex items-center justify-center flex-shrink-0">
                            <svg class="w-4.5 h-4.5 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form method="POST" action="{{ route('profile.visibility.update') }}" class="space-y-5">
                        @csrf
                        <div>
                            <label class="block text-xs font-semibold text-fg-muted mb-2">Who can see your profile</label>
            ...
            <div x-show="activeTab === 'security'"
                ...
                <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-7 mb-5">
                    <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                        ...
                        <div>
                            <h2 class="text-base font-semibold text-fg-strong">Login Alerts</h2>
                            <p class="text-xs text-fg-muted">Get notified when your account is accessed</p>
                        </div>
                    </div>
                    <form method="POST" action="{{ route('profile.security.alerts') }}" class="space-y-3">
                        ...
                        <label class="flex items-center justify-between gap-4 p-4 rounded-xl border border-edge hover:border-edge-strong hover:bg-surface-hover cursor-pointer transition-all">
                            <div>
                                ...
                                <p class="text-xs text-fg-muted mt-0.5">{{ $desc }}</p>
                            </div>
                ...
                <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-7 mb-5"
                    ...
                    <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                        ...
                        <div class="flex-1">
                            <h2 class="text-base font-semibold text-fg-strong">Change Password</h2>
                            <p class="text-xs text-fg-muted">
                                Last changed: <span class="text-fg-muted">{{ $user->password_changed_at?->diffForHumans() ?? 'Never' }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('profile.password') }}">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input :type="{{ $showVar }} ? 'text' : 'password'" name="{{ $name }}"
                                        class="w-full pr-10 px-4 py-3 rounded-xl bg-surface-raised border @error($name) border-red-500/50 @else border-edge @enderror text-fg placeholder-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/25 focus:border-amber-500/30 transition-all"
                                        placeholder="{{ $placeholder }}" autocomplete="{{ $autocomplete }}">
                                    <button type="button" @click="{{ $showVar }} = !{{ $showVar }}"
                                        class="absolute right-3 top-1/2 -translate-y-1/2 text-fg-muted hover:text-fg-muted transition-colors">
                        ...
                        <div class="p-4 rounded-xl bg-surface-raised border border-edge mb-5">
                            <p class="text-xs text-fg-muted font-semibold mb-2.5">Password requirements</p>
                ...
                <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-7 mb-5">
                    <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                        <div class="w-9 h-9 rounded-xl bg-surface-hover border border-edge flex items-center justify-center flex-shrink-0">
                            <svg class="w-4.5 h-4.5 text-fg-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <div>
                            <h2 class="text-base font-semibold text-fg-strong">Recent Login Activity</h2>
                            <p class="text-xs text-fg-muted">Last 10 login attempts on your account</p>
                        </div>
                    </div>
                    ...
                    <div class="py-10 text-center">
                        ...
                        <p class="text-sm text-fg-muted">No login history yet.</p>
                    </div>
                    @else
                    <div class="space-y-1">
                        @foreach($loginHistory as $log)
                        <div class="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-surface-hover transition-colors border border-transparent hover:border-edge-strong">
                            <div class="flex items-center gap-3">
                                <div class="w-2 h-2 rounded-full flex-shrink-0 {{ $log->isSuccessful() ? 'bg-emerald-400' : 'bg-red-400' }}"></div>
                                <div>
                                    <p class="text-sm text-fg-muted">{{ $log->browser }} on {{ $log->platform }}</p>
                                    <p class="text-xs text-fg-muted">{{ $log->ip_address }} · {{ $log->device_type }}</p>
                                </div>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="text-xs text-fg-muted">{{ $log->logged_in_at?->diffForHumans() }}</p>
                                <span class="text-xs px-2 py-0.5 rounded-full {{ $log->isSuccessful() ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400' : 'bg-red-500/15 text-red-600 dark:text-red-400' }}">
                                    {{ ucfirst($log->status) }}
                                </span>
                            </div>
                        </div>
                ...
                {{-- Active Sessions --}}
                <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-7"
                    x-data="{
                    ...
                    <div class="flex items-center justify-between gap-4 mb-6 pb-5 border-b border-edge">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-xl bg-indigo-500/15 border border-indigo-500/25 flex items-center justify-center flex-shrink-0">
                                <svg class="w-4.5 h-4.5 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div>
                                <h2 class="text-base font-semibold text-fg-strong">Active Sessions</h2>
                                <p class="text-xs text-fg-muted">Devices signed in to your account</p>
                            </div>
                        </div>
                        <button @click="revokeAll()"
                            :disabled="revokingAll || sessions.filter(s => !s.is_current).length === 0"
                            class="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold text-red-600 dark:text-red-400 border border-red-500/30 hover:bg-red-500/10 disabled:opacity-40 disabled:cursor-not-allowed transition">
                    ...
                    <div x-show="flashMsg" x-transition
                        ...
                        :class="flashType === 'success' ? 'bg-emerald-500/10 border border-emerald-500/25 text-emerald-600 dark:text-emerald-300' : 'bg-red-500/10 border border-red-500/25 text-red-600 dark:text-red-300'"
                        style="display:none">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-2.5">
                        <template x-for="session in sessions" :key="session.id">
                            <div class="group flex items-center justify-between gap-4 p-4 rounded-xl border transition-all"
                                :class="session.is_current ? 'border-indigo-500/25 bg-indigo-500/[0.04]' : 'border-edge bg-surface-raised hover:border-edge-strong hover:bg-surface-hover'">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-lg"
                                        :class="session.is_current ? 'bg-indigo-500/15 border border-indigo-500/25' : 'bg-surface-raised border border-edge'">
                                        <span x-text="deviceIcon(session.device_type)"></span>
                                    </div>
                                    <div class="min-w-0">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            <p class="text-sm font-medium text-fg truncate" x-text="session.browser + ' on ' + session.platform"></p>
                                            <span x-show="session.is_current"
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30 flex-shrink-0">
                                ...
                                <button x-show="!session.is_current" @click="revokeSession(session.id)" :disabled="revoking === session.id"
                                    class="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-red-600 dark:text-red-400 border border-red-500/20 hover:bg-red-500/10 hover:border-red-500/40 disabled:opacity-50 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <div x-show="sessions.length === 0" class="py-10 text-center">
                            ...
                            <p class="text-sm text-fg-muted">No active sessions found.</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 flex items-start gap-3 p-4 rounded-xl bg-amber-500/5 border border-amber-500/15">
                        ...
                        <p class="text-xs text-amber-400/80 leading-relaxed">
                            <strong class="text-amber-600 dark:text-amber-300">Security tip:</strong>
                            If you see an unfamiliar session, revoke it immediately and
                            <a href="{{ route('profile.show') }}#security" class="underline decoration-dotted hover:text-amber-600 hover:dark:text-amber-300 transition">change your password</a>.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <form method="POST" action="{{ route('profile.update') }}">
                    ...
                    <div class="rounded-2xl border border-edge bg-surface-raised backdrop-blur-xl p-7">
                        <div class="flex items-center gap-3 mb-6 pb-5 border-b border-edge">
                            ...
                            <div>
                                <h2 class="text-base font-semibold text-fg-strong">Notification Preferences</h2>
                                <p class="text-xs text-fg-muted">Control which notifications you receive</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-5 rounded-xl border border-edge hover:border-edge-strong hover:bg-surface-hover transition-all"
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-fg">{{ $label }}</p>
                                        <p class="text-xs text-fg-muted mt-0.5">{{ $description }}</p>
                                    </div>
                                </div>
                                <label class="relative flex items-center cursor-pointer flex-shrink-0 ml-4">
                                    ...
                                    <div class="w-11 h-6 bg-surface-hover border border-edge-strong rounded-full peer-checked:bg-indigo-600 peer-checked:border-indigo-500 transition-all"></div>

<div class="space-y-6">
    ...
    <section class="rounded-2xl border border-white/[.08] bg-white/[.025] p-5 md:p-6">
        ...
        @if($dashboard->learningJourney)
            ...
            <div class="mt-5 grid gap-3 md:grid-cols-2">@if($journey['next_milestone'])<div class="rounded-xl bg-white/[.035] p-4"><p class="text-xs uppercase tracking-wide text-slate-500">Next milestone</p><p class="mt-2 text-sm font-semibold text-white">{{ $journey['next_milestone'] }}</p></div>@endif @if($journey['last_review_at'])<div class="rounded-xl bg-white/[.035] p-4"><p class="text-xs uppercase tracking-wide text-slate-500">Latest progress review</p><p class="mt-2 text-sm text-slate-300">{{ $journey['last_review'] ?: 'Review completed' }}</p><p class="mt-2 text-xs text-slate-500">{{ $journey['last_review_at']->format('M j, Y') }}</p></div>@endif</div>

...
| **Academic Taxonomy** | Categories, subjects, subject topics, academic levels, skill levels, instructor topic coverage | `AcademicCategory`, `Subject`, `SubjectTopic`, `AcademicLevel`, `SkillLevel`, `InstructorSubjectTopic` | Configure taxonomy → link to instructors/pricing | None (reference data) | `filament.admin.resources.academic.*` | `ViewAny:Subject` etc. | **Implemented** — reference data, not dashboard content |

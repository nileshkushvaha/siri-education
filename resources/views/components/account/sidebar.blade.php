...
@php
    $container = $drawer
        ...
        : 'max-h-[calc(100vh-7rem)] overflow-y-auto rounded-2xl border border-edge bg-surface-raised p-3';
@endphp
    ...
    <nav class="{{ $container }}" aria-label="Account navigation">
        @foreach($menu as $group)
            <section class="mb-5" aria-labelledby="account-nav-group-{{ Str::slug($group['label']) }}-{{ $drawer ? 'drawer' : 'desktop' }}">
                <h2 id="account-nav-group-{{ Str::slug($group['label']) }}-{{ $drawer ? 'drawer' : 'desktop' }}"
                    class="mb-1 px-3 text-[11px] font-bold uppercase tracking-[0.14em] text-fg-faint">
                    {{ $group['label'] }}
                </h2>
                <div class="space-y-1">
                    @foreach($group['items'] as $item)
                        ...
                        <a href="{{ $item['url'] }}"
                           ...
                           class="{{ $active ? 'account-menu-active border-[rgb(var(--portal-a)/.28)] bg-gradient-to-r from-[rgb(var(--portal-a)/.16)] to-[rgb(var(--portal-c)/.07)] text-fg-strong' : 'border-transparent text-fg-muted hover:bg-surface-hover hover:text-fg-strong' }} flex min-h-11 items-center gap-3 rounded-xl border px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400">
        ...
        <div class="border-t border-edge pt-3">
            <form method="POST" action="{{ route('auth.logout') }}">
                @csrf
                <button type="submit" class="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-fg-muted transition-colors hover:bg-rose-400/10 hover:text-rose-600 hover:dark:text-rose-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-300">

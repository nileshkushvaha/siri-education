<div>
    <x-account.card>
        @forelse($classes as $booking)
            <div wire:key="upcoming-class-{{ $booking->id }}" class="flex items-center justify-between py-4 {{ !$loop->last ? 'border-b border-edge' : '' }}">
                <div class="min-w-0">
                ...
                <div class="text-right flex-shrink-0 ml-4">
                    ...
                    @if($booking->meeting_url)
                        <a href="{{ $booking->meeting_url }}" target="_blank" rel="noopener"
                           class="inline-block mt-2 text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-600 hover:dark:text-indigo-300">
                            Join Class →
                        </a>
                    @endif
                </div>

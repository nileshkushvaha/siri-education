<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        @forelse ($balances as $balance)
            <x-account.card>
                <p class="text-xs font-semibold uppercase tracking-wide text-fg-muted">Available ({{ $balance->currencyCode }})</p>
                ...
                @if (! $balance->canWithdraw && $balance->blockingReason)
                    <p class="mt-2 text-xs text-amber-600 dark:text-amber-300">{{ $balance->blockingReason }}</p>
                @endif
            </x-account.card>
        @empty
            <x-account.card class="sm:col-span-3">
                <div class="flex flex-col items-center justify-center py-8 text-center">
                    <h3 class="text-fg-muted font-semibold mb-2">No released earnings yet</h3>
                    <p class="text-fg-muted text-sm max-w-xs">Earnings become available for withdrawal after their hold period ends.</p>
                </div>
            </x-account.card>
        @endforelse
    </div>
    ...
    <div class="flex items-center justify-between">
        <h2 class="text-sm font-semibold uppercase tracking-wide text-fg-muted">Withdrawal requests</h2>
    ...
    @unless ($eligible)
        <x-account.card>
            <div class="flex flex-col items-center justify-center py-8 text-center">
                <h3 class="text-fg-muted font-semibold mb-2">Withdrawals not available yet</h3>
    ...
    @if ($showForm)
        <x-account.card>
            @if (! $confirming)
                <h3 class="text-fg-strong font-semibold mb-4">Request a withdrawal</h3>

                @if ($methods->isEmpty())
                    <p class="text-sm text-fg-muted">
                        You need a <a href="{{ route('dashboard.instructor.payout-methods') }}" class="text-indigo-600 dark:text-indigo-300 underline">verified payout method</a> before you can request a withdrawal.
                    </p>
                @else
                    <form wire:submit="confirm" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Payout method</label>
                            ...
                            @error('payoutMethodId') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Amount</label>
                            <input type="text" wire:model="amount" inputmode="decimal" placeholder="0.00" autocomplete="off"
                                class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none" />
                            @error('amount') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                        </div>

                        <div class="sm:col-span-2">
                            <label class="block text-xs font-semibold uppercase tracking-wide text-fg-muted mb-1">Note (optional)</label>
                            <input type="text" wire:model="note" maxlength="500" autocomplete="off"
                                class="w-full rounded-xl border border-edge bg-surface-raised px-3 py-2 text-sm text-fg-strong focus:border-indigo-400 focus:outline-none" />
                            @error('note') <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p> @enderror
                        </div>
                ...
                @endif
            @else
                <h3 class="text-fg-strong font-semibold mb-4">Confirm your withdrawal</h3>
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        <dt class="text-fg-muted">Amount</dt>
                        <dd class="text-fg-strong font-semibold">{{ $amount }} {{ $selectedMethod?->currency_code }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-fg-muted">Destination</dt>
                        <dd class="text-fg-strong">{{ $selectedMethod?->display_label }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-fg-muted">Fee</dt>
                        <dd class="text-fg-strong">{{ $selectedMethod ? \App\Support\MoneyFormatter::format(0, $selectedMethod->currency_code) : '—' }}</dd>
                    </div>
                </dl>
    ...
    <x-account.card>
        @forelse ($withdrawals as $withdrawal)
            <div wire:key="withdrawal-{{ $withdrawal->id }}" class="py-4 {{ ! $loop->last ? 'border-b border-edge' : '' }}">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <p class="text-sm font-medium text-fg-strong font-mono">{{ $withdrawal->reference }}</p>
                        ...
                        @if ($withdrawal->status === \App\Earnings\Enums\InstructorWithdrawalStatus::Rejected && $withdrawal->rejection_reason)
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">Rejected: {{ $withdrawal->rejection_reason }}</p>
                        @elseif ($withdrawal->status === \App\Earnings\Enums\InstructorWithdrawalStatus::Failed)
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $withdrawal->failure_reason ?: 'This payout could not be completed. Our team has been notified.' }}</p>
                        @elseif ($withdrawal->status === \App\Earnings\Enums\InstructorWithdrawalStatus::Reversed)
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">This payout was returned by the receiving bank after being paid. Our finance team is reviewing it.</p>
                        @elseif ($withdrawal->status === \App\Earnings\Enums\InstructorWithdrawalStatus::Processing)
                            <p class="mt-1 text-xs text-fg-muted">Your payout is being processed.</p>
                        @elseif ($withdrawal->status === \App\Earnings\Enums\InstructorWithdrawalStatus::Paid)
                            <p class="mt-1 text-xs text-emerald-600 dark:text-emerald-300">Paid{{ $withdrawal->paid_at ? ' on '.viewer_date($withdrawal->paid_at) : '' }}.</p>
                        @endif
                    </div>
        ...
        @empty
            <div class="flex flex-col items-center justify-center py-12 text-center">
                <h3 class="text-fg-muted font-semibold mb-2">No withdrawal requests yet</h3>
                <p class="text-fg-muted text-sm max-w-xs">When you request a withdrawal it will appear here with its live status.</p>
            </div>
        @endforelse

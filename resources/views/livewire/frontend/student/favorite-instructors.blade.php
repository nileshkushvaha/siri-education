    ...
    <x-account.card title="Favorite Instructors">
        @forelse($favorites as $instructor)
            <div wire:key="favorite-instructor-{{ $instructor->id }}" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-4 {{ !$loop->last ? 'border-b border-white/[0.06]' : '' }}">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-white truncate">{{ $instructor->name }}</p>
                        <p class="text-xs text-slate-400 truncate">{{ $instructor->profile?->headline ?? 'Instructor' }}</p>
                    </div>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('instructors.show', $instructor) }}" class="px-3 py-2 rounded-lg border border-white/[0.10] text-xs font-semibold text-slate-300 hover:text-white hover:bg-white/[0.05]">
                        View
                    </a>
                    <button type="button" wire:click="remove({{ $instructor->id }})" class="px-3 py-2 rounded-lg border border-rose-500/20 text-xs font-semibold text-rose-200 hover:bg-rose-500/10">
                        Remove
                    </button>
                </div>
            </div>
        @empty
            <div class="py-12 text-center">
                ...
                <p class="text-sm font-semibold text-slate-300">No favorite instructors yet.</p>
                <p class="text-sm text-slate-500 mt-1">Browse approved instructors and save the ones you want to learn with.</p>

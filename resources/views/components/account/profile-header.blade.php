...
@if($variant === 'full')
    ...
    <div class="relative rounded-2xl border border-edge overflow-hidden mb-6" data-account-profile-header data-variant="full">
        {{-- Cover --}}
            ...
            <div class="-mt-10 flex flex-col items-start gap-5 sm:-mt-12 sm:flex-row sm:items-start sm:gap-6">
                <div class="relative z-20 group flex-shrink-0">
                    <div class="w-20 h-20 rounded-2xl overflow-hidden border-4 border-surface-dark bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-xl shadow-indigo-500/20">
                        ...
                        @if(!isset($avatar))
                            ...
                            @else
                                <span class="text-3xl font-bold text-fg-strong">{{ $summary->initial }}</span>
                            @endif
                        @endif
                    </div>
                    {{ $avatarActions ?? '' }}
                ...
                <div class="min-w-0 flex-1 sm:mt-14">
                    <div class="flex flex-wrap items-center gap-3 mb-1">
                        <h1 class="text-2xl font-bold text-fg-strong">{{ $summary->name }}</h1>
                        @if($summary->emailVerified)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/25">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                                Verified
                            </span>
                        @endif
                    </div>
                    <p class="text-fg-muted text-sm mb-3">{{ $summary->email }}</p>
                ...
                <div class="hidden flex-shrink-0 flex-col items-end gap-2 lg:mt-14 lg:flex">
                    {{ $actions ?? '' }}
                    <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface-raised border border-edge">
                        <span class="w-2 h-2 rounded-full {{ $summary->online ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600' }}"></span>
                        <span class="text-xs text-fg-muted">{{ $summary->online ? 'Online' : 'Offline' }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div data-account-profile-header data-variant="compact">
        <div class="flex items-center gap-3 mb-4">
            <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center overflow-hidden flex-shrink-0">
                @if($summary->avatarThumbUrl)
                    <img src="{{ $summary->avatarThumbUrl }}" class="w-full h-full object-cover" alt="{{ $summary->name }}">
                @else
                    <span class="text-fg-strong font-bold text-lg">{{ $summary->initial }}</span>
                @endif
            </div>
            <div>
                <p class="text-fg-strong font-semibold text-sm">{{ $summary->name }}</p>
                <div class="flex items-center gap-1.5 mt-0.5">
                    <span class="w-1.5 h-1.5 rounded-full {{ $summary->online ? 'bg-emerald-400' : 'bg-slate-600' }}"></span>
                    <span class="text-xs {{ $summary->online ? 'text-emerald-600 dark:text-emerald-400' : 'text-fg-muted' }}">{{ $summary->online ? 'Active' : 'Offline' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-2 mb-4">
            <div class="flex items-center justify-between text-xs">
                <span class="text-fg-muted">Profile Complete</span>
                <span class="text-fg-muted font-medium">{{ $summary->profileCompletion }}%</span>
            </div>
            ...
            <p class="text-fg-muted text-xs">Add your photo, bio and preferences to complete your profile</p>
        </div>

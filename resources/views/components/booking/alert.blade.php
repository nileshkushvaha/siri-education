    ...
    <div class="flex-1 text-sm text-red-700">
        ...
        @if($retry)
            <button type="button" @click="{{ $retry }}"
                    class="mt-1.5 inline-flex items-center gap-1 text-xs font-semibold text-red-800 underline underline-offset-2 hover:text-red-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-300 rounded">
                Try again
            </button>
        @endif
    </div>
    <button type="button" @click="banner = ''" aria-label="Dismiss error"
            class="text-red-400 hover:text-red-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-300 rounded">
